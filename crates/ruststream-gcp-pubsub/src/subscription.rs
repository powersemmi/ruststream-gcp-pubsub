...
/// there:
///
/// * [`batch_wait`](PubSubSubscription::batch_wait) is honoured. Batching is on the client either
///   way, over the framework's own buffer, so the deadline means the same thing here.
/// * [`create_with_topic`](PubSubSubscription::create_with_topic) is ignored. The stand-in holds
...
///   emulator instead.
/// * [`max_outstanding`](PubSubSubscription::max_outstanding) is ignored. It is the streaming
///   pull's flow control, and there is no pull here: the router hands a delivery straight to the
///   subscription's queue.
/// * [`ack_extension`](PubSubSubscription::ack_extension) is ignored. Nothing leases a message in
...
/// ```
#[cfg(feature = "testing")]
impl SubscriptionSource<crate::testing::ConnectedPubSubTestBroker> for PubSubSubscription {
