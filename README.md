...
- **Ordering keys as the partition key.** A publish names its key with `with_ordering_key`, this crate's step on the framework's publish builder; the key travels as the `partition-key` header and comes back as the same header (feeding `Partitioned`) on delivery.
