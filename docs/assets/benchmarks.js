...
(() => {
  ...
  function overhead(verdict, percent, labels) {
    if (verdict === "indistinguishable") {
      return labels.indistinguishable;
    }
    return typeof percent === "number" ? (percent >= 0 ? "+" : "") + percent + "%" : "-";
  }
  ...
  function scenarios(results, labels, lang) {
    ...
    for (const scenario of results.scenarios) {
      ...
      const paced = scenario.broker_bound ? " (" + labels.brokerBound + ")" : "";
      ...
      row.appendChild(
        text(
          "td",
          overhead(scenario.adapter_verdict, scenario.adapter_overhead_percent, labels) + paced,
        ),
      );
      row.appendChild(
        text("td", overhead(scenario.verdict, scenario.overhead_percent, labels) + paced),
      );
    }
    return element;
  }
