# Contributing to ruststream-gcp-pubsub

`ruststream-gcp-pubsub` is the Google Cloud Pub/Sub broker crate of RustStream. The framework's core crate lives in
...
git clone https://github.com/powersemmi/ruststream-gcp-pubsub.git
```
...
- **Docker** with Compose, for the Pub/Sub emulator stand the live suite and the benchmarks run against.
- Per task:
...
just test-brokers   # the live suite against the Pub/Sub emulator
...
starts the Compose stand, runs the whole suite against the emulator with the live tests required,
and stops the stand.
...
From the core, `just brokers gcp-pubsub` runs the same command against the core's working tree.
...
```bash
just brokers-up
PUBSUB_TEST_HOST=127.0.0.1:8085 RUSTSTREAM_REQUIRE_LIVE=1 \
